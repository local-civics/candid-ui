 ...
 * AssignmentPageProps
 */
export type AssignmentPageProps = AssignmentModel & {
  isLoading?: boolean
};
...
export function AssignmentPage(props: AssignmentPageProps) {
  ...
  return <Container size="lg" pb="xl">
    <Stack spacing={10}>
      ...
      <Tabs defaultValue={!!props.questions?.length ? "summary" : "breakdown"}>
        <Tabs.List>
          { !!props.questions?.length && <Tabs.Tab icon={<IconChartArrows/>} value="summary">
            <Text px={15} py={10}>
              Summary
            </Text>
          </Tabs.Tab> }
        ...
        { !!props.questions?.length && <Tabs.Panel value="summary" pt="xs">
