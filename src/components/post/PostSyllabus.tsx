...
import {
    Container, Grid, Text, Title, Stack, Group, Button,
    createStyles,
} from '@mantine/core';
import {
    IconClockFilled,
...
export function PostSyllabus(props: PostSyllabusProps){
    ...
    const sessions = props.syllabusItems.map((v, i) => {
        return <>
            <Grid.Col span={3} pr="9rem">
                <Text size="sm" w="max-content" mx="auto">SESSION</Text>
                <Text mt={5} w="max-content" mx="auto" weight={700} size="2rem">{i + 1}</Text>
            </Grid.Col>
    ...
    return <Container fluid p={25} mx={0} className={classes.wrapper}>
